...
/*
	@type: CLASS
	@author: MARIZ MELO
	@description: Native functions to deal with database connection 
*/	

class Database_native extends Debug{
	...
	function __construct($debug=0, $debugphp=0){
		...
		if($debug){
			...
			$this->debugMESSAGE('S', 'DATABASE NATIVE object created');
		}
		return '';
	...
	function __toString(){
		...
		$HELP  = 'type: DATABASE NATIVE Class\n'
		. 'author:	Mariz Melo\n'
		...
		if(isset($url) && isset($user) && isset($pass) && isset($database)){
			...
			switch($db['sgdb']){
				case 'mysql':	if (!extension_loaded('mysql')){
								...
								include_once('./hanoi/core/Database_native/db_mysql.php');
								break;
								
				/*case 'oracle':	include_once('./xcore/php/Database_native/db_oracle.php');
								break;
								
				case 'interbase':	include_once('./xcore/php/Database_native/db_interbase.php');
								break;
								
				case 'firebird':	include_once('./xcore/php/Database_native/db_firebird.php');
								break;
								
				case 'sqlserver':	include_once('./xcore/php/Database_native/db_sqlserver.php');
								break;
								
				case 'sqllite':	include_once('./xcore/php/Database_native/db_sqllite.php');
								break;*/
			}//switch
